...
describe("live sweep", () => {
  ...
  let link: PayoutLinkCreated | undefined;
  ...
  beforeAll(async () => {
    ...
    await ob.webhooks.register({ url: HOOK }); // a per-invoice url_callback needs a registered endpoint (it signs with its secret)
  ...
  it("catalog & account", async () => {
    expect((await pub.checkout.listCurrencies()).currencies.length).toBeGreaterThan(0);
    expect((await pub.account.listExchangeRates({ currency_from: "BTC" })).items).toBeDefined();
    expect((await ob.account.getBalance()).balance.merchant).toBeDefined();
    expect(typeof (await ob.referrals.getInfo()).code).toBe("string");
    expect(typeof (await ob.settings.configureVrcs()).enabled).toBe("boolean");
    expect(typeof (await ob.settings.configureVrcs({ enabled: false })).enabled).toBe("boolean");
  });

  it("payments: lookups, qr, services, public checkout, batch", async () => {
    expect((await ob.payments.getInfo({ uuid: invoice.uuid })).uuid).toBe(invoice.uuid);
    ...
    expect(typeof (await ob.payments.getQr({ uuid: invoice.uuid })).image).toBe("string");
    expect((await ob.payments.listServices({ limit: 5 })).items.length).toBeGreaterThan(0);
    expect((await pub.checkout.get(invoice.uuid)).status).toBe("created");
    expect(typeof (await pub.checkout.getQr(invoice.uuid)).image).toBe("string");
    ...
    expect(
      (await pub.checkout.selectMethod(multi.uuid, { currency: "USDT", network: "tron" })).network,
    ).toBe("tron");
    await accept(ob.webhooks.resendPayment({ uuid: invoice.uuid }));
    await accept(ob.payments.sendEmail({ uuid: invoice.uuid }));
    const b = await ob.batches.createPayment({
      on_error: "continue",
      payments: [
    ...
    expect((await ob.batches.getInfo({ batch_id: b.batch_id })).batch_id).toBe(b.batch_id);
    ...
    expect((await ob.payments.cancel({ uuid: toCancel.uuid })).status).toBe("cancelled");
    for await (const p of ob.payments.listHistory({ limit: 2 })) expect(p.uuid).toBeTruthy();
  });
  ...
  it("deposit → paid → refund, resolve, refund batch", async () => {
    ...
    await accept(ob.payments.resolve({ uuid: invoice.uuid, action: "accept" }));
    await accept(
      ob.batches.createRefund({
        refunds: [
  ...
  it("payouts: calculate, validate, create, info, cancel, mass, batch, services, fee configs", async () => {
    ...
    expect((await ob.payouts.getInfo({ order_id: po.order_id ?? "" })).uuid).toBe(po.uuid);
    await accept(ob.payouts.cancel({ uuid: po.uuid }));
    await accept(ob.payouts.approve({ uuid: po.uuid }));
    const mass = await ob.payouts.createMass({
      payouts: [
        {
          amount: "5",
          currency: "USDT",
    ...
    const batch = await ob.batches.createPayout({
      payouts: [
        {
          amount: "5",
          currency: "USDT",
    ...
    expect((await ob.payouts.listServices()).items.length).toBeGreaterThan(0);
    expect(
      typeof (await ob.settings.setPayoutFeeConfig({ fee_on_recipient: true })).fee_on_recipient,
    ).toBe("boolean");
    expect(typeof (await ob.settings.getPayoutFeeConfig()).fee_on_recipient).toBe("boolean");
    expect(
      typeof (await ob.settings.setRefundFeeConfig({ fee_on_customer: true })).fee_on_customer,
    ).toBe("boolean");
    expect(typeof (await ob.settings.getRefundFeeConfig()).fee_on_customer).toBe("boolean");
    expect((await ob.payouts.listHistory({ kind: "refund", limit: 5 })).items).toBeDefined();
  });
  ...
  it("payout links: create, info, list, claim preview/claim, cancel, batch", async () => {
    ...
    expect((await ob.payoutLinks.get({ link_id: link.link_id })).status).toBe("funded");
    expect((await ob.payoutLinks.list({ limit: 5 })).items.length).toBeGreaterThan(0);
    expect((await pub.payoutLinks.getPayoutClaim(link.claim_token)).claimable).toBe(true);
    const claimed = await pub.payoutLinks.claimPayout(link.claim_token, { address: ADDR });
    ...
    expect((await ob.payoutLinks.cancel({ link_id: second.link_id })).status).toBe("cancelled");
    const batch = await ob.payoutLinks.createBatch({
      items: [
  ...
  it("payment links: create, info, list, toggle, public view, checkout", async () => {
    ...
    expect((await ob.paymentLinks.get({ link_id: created.link_id })).active).toBe(true);
    expect((await ob.paymentLinks.list()).items.length).toBeGreaterThan(0);
    expect((await pub.checkout.getPublicPaymentLink(created.link_id)).amount_mode).toBe("fixed");
    expect(
      (await pub.checkout.paymentLink(created.link_id, { currency: "USDT", network: "tron" })).uuid,
    ).toBeTruthy();
    expect((await ob.paymentLinks.toggle({ link_id: created.link_id, active: false })).active).toBe(
      false,
    );
  });
  ...
  it("splits & settings", async () => {
    ...
    expect((await ob.splits.setRecipientOptIn({ enabled: true })).enabled).toBe(true);
    expect((await ob.splits.getRecipientOptIn()).enabled).toBe(true);
    expect((await ob.splits.deleteRule({ rule_id: rule.rule_id })).ok).toBe(true);
    expect(
    ...
    expect(
      (
        await ob.settings.setAcceptedCurrencies({
          accepted: [{ currency: "USDT", network: "tron" }],
        })
      ).ok,
    ).toBe(true);
    expect((await ob.settings.listAcceptedCurrencies()).items).toBeDefined();
    expect(
    ...
    expect(
      (
        await ob.settings.setAutoWithdrawRule({
          currency: "USDT",
          network: "tron",
          address: ADDR,
          min_amount: "100",
    ...
    expect(await ob.settings.listAutoWithdrawRules()).toBeDefined();
    expect(await ob.settings.deleteAutoWithdrawRule({ currency: "USDT" })).toBeDefined();
    expect((await ob.apiAllowlist.addEntry({ cidr: "203.0.113.0/24" })).items).toContain(
      "203.0.113.0/24",
    );
    expect((await ob.apiAllowlist.list()).items).toContain("203.0.113.0/24");
    expect((await ob.apiAllowlist.setEnabled({ enabled: false })).enabled).toBe(false);
    expect((await ob.apiAllowlist.removeEntry({ cidr: "203.0.113.0/24" })).items).not.toContain(
      "203.0.113.0/24",
    );
  });

  it("webhooks & sandbox inspector", async () => {
    const ep = await ob.webhooks.register({ url: HOOK });
    expect(ep.endpoint_id).toBeTruthy();
    const rotated = await ob.webhooks.rotateSecret();
    expect(rotated.secret).toBeTruthy();
    expect((await ob.webhooks.listDeliveries({ limit: 5 })).items).toBeDefined();
    await accept(
      ob.webhooks.sendTestPayment({
        url_callback: HOOK,
        currency: "USDT",
        network: "tron",
        status: "paid",
    ...
    await accept(ob.webhooks.sendLegacyTest({ url: HOOK, status: "paid" }));
    const inspector = await ob.sandbox.listWebhooks({ limit: 5 });
    expect(inspector.items).toBeDefined();
    const terminal = inspector.items.find((d) => d.status === "delivered" || d.status === "dead");
    if (terminal) await accept(ob.sandbox.replayWebhook({ delivery_id: terminal.id }));
  });
  ...
  it("wallets (refused for a dev store, as documented)", async () => {
    ...
    await accept(ob.wallets.getQr({ address: ADDR }));
    await accept(ob.wallets.block({ address: ADDR }));
    await accept(ob.payouts.transferToPersonal({ amount: "5", currency: "USDT" }));
  });

  it("documents (when the stand has a renderer)", async () => {
    if (!docsEnabled) return;
    const pdf = await ob.documents.getStatement({
      from: "2026-01-01",
      to: "2026-12-31",
      lang: "en",
    });
    ...
    expect((await ob.documents.getFees()).content.length).toBeGreaterThan(0);
    expect((await ob.documents.getLedger({ format: "csv" })).contentType).toMatch(/csv|pdf/);
    if (link)
      expect(
        (await ob.documents.getPayoutLinkCheque({ claim_token: link.claim_token!, lang: "en" }))
          .contentType,
      ).toMatch(/pdf/);
    ...
    expect((await ob.documents.getJob({ job_id: job.job_id })).job_id).toBe(job.job_id);
    await accept(ob.documents.downloadJobFile({ job_id: job.job_id }));
    const info = await ob.payments.getInfo({ uuid: invoice.uuid });
    const u = new URL(info.document_url);
    expect(
      (
        await pub.documents.getSigned(u.pathname.split("/")[3]!, u.pathname.split("/")[4]!, {
