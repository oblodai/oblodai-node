...
describe("a float in an amount fails before the network (spec §3.3)", () => {
  ...
  // Needs the backend's contract; a configured backend without one is an error, not a skip.
  it.skipIf(!hasBackend() && !process.env.OBLODAI_BACKEND)(
    "the not-money set is exactly the fractional numbers of the contract's request bodies",
    () => {
