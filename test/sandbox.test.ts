...
describe('песочница (v1.2.0)', () => {
  ...
  it('simulateDeposit без опциональных полей шлёт только invoice_id (бэкенд-дефолты)', async () => {
    const { fn, calls } = mockFetch([
      {
        status: 200,
        body: { state: 0, result: { invoice_id: 'inv-1', txid: 'gen', amount: '10', confirmations: 0 } },
      },
    ]);
    const client = makeClient(fn);
  ...
  it('faucet шлёт {asset, amount, idempotency_key} на /v1/sandbox/faucet', async () => {
    const { fn, calls } = mockFetch([
      { status: 200, body: { state: 0, result: { asset: 'USDT', amount: '500', journal_id: 'j-1' } } },
    ]);
    const client = makeClient(fn);

    const res = await client.sandbox.faucet({ asset: 'USDT', amount: '500', idempotency_key: 'k-1' });

    expect(calls[0]!.url).toBe('https://api.test/v1/sandbox/faucet');
    expect(calls[0]!.init.method).toBe('POST');
    expect(sentBody(calls[0]!.init)).toEqual({ asset: 'USDT', amount: '500', idempotency_key: 'k-1' });
